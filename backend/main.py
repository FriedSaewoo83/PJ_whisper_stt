...
# 웹 브라우저(클라이언트) → 음성파일 업로드 → 분석클릭 → API → Whisper → 결과 → 웹 브라우저(클라이언트)
# POST, http://127.0.0.1/stt/whisper, input(음성파일)
@app.post("/stt/whisper")
async def wtt_wisper(file: UploadFile):  # 음성파일
    # 사용자가 업로드 한 음성파일 저장
    save_dir = "save"
    ...
    # 음성파일 불러오기
    file=open("save/{file.filename}", "rb")
...
async def stt_clova(file:UploadFile):
    ...
    # API -> Client(Streamlit)
    # return값 JSON -> String
    # Streamlit은 response.text
    return {"status_code" : 200,
            "STT" : response.text}
